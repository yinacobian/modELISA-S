...
IgG_plot <- all_data %>%
  ...
  coord_equal() +
  facet_wrap(~ patient)
ggsave('IgG_plot.pdf',plot = IgG_plot, width = 15.2, height = 13.9, units = 'in', scale = 1.5)
...
IgA_plot <- all_data %>%
  ...
  coord_equal() +
  facet_wrap(~ patient)
ggsave('IgA_plot.pdf',plot = IgA_plot, width = 15.2, height = 13.9, units = 'in', scale = 1.5)
